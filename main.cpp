...
public:
	...
	Field(char _ss[20][81]){
		...
		fgolds[45] = 1;
		fgolds[17] = fgolds[18] = fgolds[53] = 1; sum = 3;
	...
	int GetLength(){
		...
		for(int i=ii; i<(int)golds.size(); i+=10)
			if(!fgolds[i] && vp.size()<10) vp.push_back(golds[i]);
		...
		do{
			...
			for(auto e : vp){
				cur += dist[p.first][p.second][e.first][e.second];
				p = e;
			}
			res = min(res, cur);
	...
	pair<int, vector<int>> GetBrute(){
		...
		do{
			if(path.size() + GetLength() > 1000) return make_pair(-4, path);
			...
			if(mret < 0){
				...
				for(; lim<=100; ++lim){
					Field cf(*this);
					int ret = cf.CarefulToGoal(p.first, p.second, lim);
					if(ret == 2) break;
				}
				if(lim > 100) return make_pair(-3, path);
				int ret = CarefulToGoal(p.first, p.second, lim);
	...
	int CarefulToGoal(int gx, int gy, int limit){
